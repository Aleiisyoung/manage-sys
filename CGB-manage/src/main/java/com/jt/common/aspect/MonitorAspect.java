 ...
 * 
 * @author 速度 通知结构关系如下： try{
 * @Before target.method();//目标对象方法执行
 * @AfterReturning }catch(Exception e){
 * @AfterThrowing throw e; }finally{
 * @After }
 * 
 */
@Order(2)
...
public class MonitorAspect {// 横切面
	...
	/**
	 * 定义切入点:PointCut(在哪些点切入扩展功能) 在当前应用中指的是对哪些方法的执行 进行业务监控 其中bean为AOP中的一个切入点表达式
	 */
