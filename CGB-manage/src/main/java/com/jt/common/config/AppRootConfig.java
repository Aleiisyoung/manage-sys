...
public class AppRootConfig {// service,dao
	...
	public DataSource newDruidDataSource(@Value("${jdbcDriver}") String driverClass,
			@Value("${jdbcUrl}") String jdbcUrl, @Value("${jdbcUser}") String username,
	...
	public DefaultWebSecurityManager newDefaultWebSecurityManager(AuthorizingRealm realm, EhCacheManager cacheManager) {
	...
	public ShiroFilterFactoryBean newShiroFilterFactoryBean(SecurityManager securityManager) {// shiro
																								// 包
	...
	@Bean("txManager")
	public DataSourceTransactionManager newDataSourceTransactionManager(@Autowired DataSource dataSource) {
